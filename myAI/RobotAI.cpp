...
void RobotAI::DirectMoveToPoint(RobotAI_Order &order,const double &x,const double&y)
{
	...
	{
	case ET_Spider:DirectMoveToPoint_Spider(order,x,y);break;
...
void RobotAI::DirectMoveInDirection(RobotAI_Order& order, const double & r)
{
	...
	{
	case ET_Spider:DirectMoveInDirection_Spider(order,r);break;
	case ET_Robotman:DirectMoveInDirection_Robotman(order,r);break;
	case ET_AFV:DirectMoveInDirection_AFV(order,r);break;
	case ET_UFO:DirectMoveInDirection_UFO(order,r);break;
	}
}
...
void RobotAI::Reload(RobotAI_Order & order)
{
	...
	*/
	...
	else
	{
		...
		else if(battle_info.arsenal[0].respawning_time==0)
		{
			select_arsenal_idx=1;
		}
		else
